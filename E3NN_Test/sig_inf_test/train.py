...
def build_data(atom, sig_text, type_encoding, type_onehot, am_onehot, r_max=5.0):
    ...
    data = tg.data.Data(pos = positions, lattice = lattice, symbol = symbols, 
                        ...
                        edge_vec=edge_vec, edge_len=edge_len, sinf = sinf)
    return data
...
for i in range(len(atoms)):
    adata = build_data(atoms[i], sig_texts[i], type_encoding, type_onehot, am_onehot, radial_cutoff)
...
scheduler = torch.optim.lr_scheduler.StepLR(opt, step_size=5, gamma=0.4)
...
save_path = "sinf_model.pth"
# model.load_state_dict(torch.load(save_path))

train(model, opt, train_data, loss_fn, scheduler, save_path= save_path, max_iter=40)
