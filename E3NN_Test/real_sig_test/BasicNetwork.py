...
def evaluate(model, dataset):
    ...
    for i in range(N1):
        for j in range(N2):
            ...
            if i == 0 and j ==0:
                axs[i,j].plot(iws[0], dataset[N2*i + j].sig[0,0].numpy(), c="black", label="True DMFT Re{$\Sigma$(i$\omega_n$)}")
                axs[i,j].plot(iws[0], output[0], c="red", marker='o', markersize=2, label="Predicted DMFT Re{$\Sigma$(i$\omega_n$)}")
            else:
