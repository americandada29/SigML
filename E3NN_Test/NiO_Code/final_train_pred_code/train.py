...
full_sig_model = get_standard_full_sig_model(n_matsubara, ave_neighbors, device=device)
opt = torch.optim.AdamW(full_sig_model.parameters(), lr=0.05, weight_decay=0.01)
...
train_full_sig(full_sig_model, opt, train_data, loss_fn, scheduler, save_path = save_path, max_iter=20, val_percent = 0.1, device=device, batch_size=4)
full_sig_model.load_state_dict(torch.load(save_path))
for o in range(5):
