...
#   all_sigs = []
#   all_iws = []
#   for fi in sorted(os.listdir("../self_energies"), key= lambda x: x.split("_")[0]) :
#     data = np.loadtxt("../self_energies/" + fi)
#     iws = data[:,0]
#     all_iws.append(iws)
...
def get_sigs():
  all_sigs = []
  all_iws = []
  sdir = "../Pred_Sinf/Sinf_Data/"
  for fi in sorted(os.listdir(sdir), key= lambda x: x.split("_")[0]) :
    # data = np.loadtxt("../self_energies/" + fi)
    with open(sdir + fi,"rb") as f:
      atoms, sigfiles = pickle.load(f)
    for k in range(len(sigfiles)):
      data = np.loadtxt(StringIO(sigfiles[k]))
      iws = data[:,0]
      all_iws.append(iws)
      sigs = np.zeros((4, 5, data.shape[0])).astype(np.complex128)  # 4 atoms, 5 orbitals, 2 numbers for real and imag parts
      for i in range(4):
...
def write_gxs():
  ...
  for a in range(len(all_sigs)):
    ...
    all_gxs.append(Gxs)   
    print(a)
  with open("../Data/all_gxs.pkl","wb") as f:
    pickle.dump(all_gxs, f)     
                

def read_gxs():
  all_iws, all_sigs = get_sigs()
  with open("../Data/old_all_gxs.pkl","rb") as f:
    all_gxs = pickle.load(f)
  return all_iws, all_gxs


def graph_high_freq_tail(popt, iws, sig):
  plt.plot(iws, sig.imag, marker="o")
  pxs = np.linspace(iws[0], iws[-1], 1000)
  psigs = high_freq_func(pxs, *popt)
  plt.plot(pxs, psigs, marker="x")
  plt.show()
  exit()

def fit_sig_tails(iws, sigs):
  all_tails = []
  for i in range(len(sigs)):
    ttails = np.zeros((4, 5, 2))
    for j in range(4):
      for k in range(5):
        tiws = iws[i][int(sigs[i].shape[2]/2):]
        tsig = sigs[i][j,k, int(sigs[i].shape[2]/2):]
