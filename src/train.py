...
# opt = torch.optim.AdamW(ef_model.parameters(), lr=0.01, weight_decay=0.05)
# scheduler = torch.optim.lr_scheduler.StepLR(opt, step_size=5, gamma=0.5)
# loss_fn = torch.nn.SmoothL1Loss(reduction="sum")
# save_path = "SAVED_MODELS/ef_model_fe2o2.pth"

# train_ef(ef_model, opt, train_data, loss_fn, scheduler, save_path = save_path, max_iter=30, val_percent = 0.1, device=device, batch_size=1)
...
opt = torch.optim.AdamW(sinf_model.parameters(), lr=0.005/16, weight_decay=0.05)
scheduler = torch.optim.lr_scheduler.StepLR(opt, step_size=5, gamma=0.25)
loss_fn = torch.nn.SmoothL1Loss(reduction="sum")
