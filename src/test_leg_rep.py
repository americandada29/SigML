...
def plot_sig(sig, atom=0):
   num_orbitals = sig.shape[-1]
   fig, axs = plt.subplots(num_orbitals)
   for i in range(num_orbitals):
      axs[i].plot(sig.real[atom, :, i], c='blue', marker='o')
      axs[i].plot(sig.imag[atom, :, i], c='red', marker='o')
      axs[i].axhline(0, color='black', linestyle='--')
   plt.show()

def compare_sigs(iws, sig1, sig2, atom=0, hfm_matrix=None, smooth=False):
   num_orbitals = sig1.shape[-1]
   fig, axs = plt.subplots(2, 3)
   for i in range(5):
      axs[i//3, i%3].plot(iws, sig1.real[atom, :, i], c='green', marker='o', markersize=2)
      axs[i//3, i%3].plot(iws, sig2.real[atom, :, i], c='blue', marker='o'  ,markersize=2)
      axs[i//3, i%3].plot(iws, sig1.imag[atom, :, i], c='red', marker='o'   ,markersize=2)
      axs[i//3, i%3].plot(iws, sig2.imag[atom, :, i], c='purple', marker='o',markersize=2)
      ...
      if hfm_matrix is not None:
         ...
         axs[i//3, i%3].plot(iws, sig2_newtail.real, c='black', marker='o', markersize=2)
         axs[i//3, i%3].plot(iws, sig2_newtail.imag, c='black', marker='o', markersize=2)
      ...
      if smooth:
         ...
         axs[i//3, i%3].plot(iws, sig2_smooth, c='black', marker='o', markersize=2)
...
#    pefs.extend(tefs)
# dataset = build_data(patoms, sig_texts=psig_texts, efs=pefs, device=device)

with open("dataset.pkl", "rb") as f:
    dataset = pickle.load(f)


test_data = dataset[49]
...
sig_l = fullatom_gl_from_giw(test_iws, test_sig, lmax=80)
# hfm_matrix = fullatom_high_freq_moments(test_iws, sig_l)
# hfm_matrix = fit_hfm(test_iws, test_sig)



recon_sig = fullatom_giw_from_gl(test_iws, sig_l, fit_tail=True, fit_tail_cutoff=35, fit_tail_start=20, fit_tail_end=35)

# hfm_matrix = fit_hfm(test_iws, recon_sig)

compare_sigs(test_iws, test_sig, recon_sig, hfm_matrix = None, smooth=False)
