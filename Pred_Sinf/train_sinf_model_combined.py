...
class SigInfModel(nn.Module):
    def __init__(self, input_length = 50, output_length = 5, num_filters=100, fc_bias=True):
...
if __name__ == "__main__":
    ...
    train_fps = fps[:N]
    train_sinfs = all_sinfs[:N]

    val_fps = fps[N:]
    val_sinfs = all_sinfs[N:]


    dataset = create_dataset(train_fps, train_sinfs, device=device)
    dataloader = DataLoader(dataset, batch_size=16, shuffle=True)

    val_dataset = create_dataset(val_fps, val_sinfs, device=device)
    val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False)


    #model = SigInfModel().to(device).double()
    model = FullCNNModel().to(device).double()
