...
def parse_sig_file(sig_text, orbital="d"):
  ...
  adjusted_sig_data = np.zeros((sig_data.shape[0], num_atoms, num_orbitals), dtype=np.complex128)
...
def get_group_dlr_coeffs(iws, S, E_max=50.0):
  S_input = np.zeros((S.shape[0], S.shape[1], S.shape[1]), dtype=np.complex128)
  ...
  w_x = d.get_dlr_frequencies()
  S_dlr = d.lstsq_dlr_from_matsubara(iws_test, S_test, beta=beta)
  S_out = np.zeros((S_dlr.shape[0], S_dlr.shape[1]), dtype=np.complex128)
...
def write_all_dlr_coeffs(sigs, E_max=130.0, fname="dlr_coeffs.pkl"):
  ...
  for sig in tqdm(sigs, desc="Creating DLR coefficients..."):
    ...
    w_x = d.get_dlr_frequencies()
    num_atoms = asig.shape[1]
    num_orbitals = asig.shape[2]
    sig_dlr = np.zeros((len(w_x), num_atoms, num_orbitals), dtype=np.complex128)
